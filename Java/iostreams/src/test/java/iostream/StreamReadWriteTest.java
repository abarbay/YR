...
public class StreamReadWriteTest {
    ...
    public void missingFileShouldThrowException() throws IllegalArgumentException {
        ...
        // When
        new iostream.StreamReadWrite().readUtfInput("data.dat", "UTF-8");

    }

    @Test
    public void testReadUtfInput() throws IOException {
        assertThat(new iostream.StreamReadWrite().readUtfInput(sourceFile.getCanonicalPath(), "UTF-8"), equalTo("árvíztűrő tükörfúrógép"));
        assertThat(new iostream.StreamReadWrite().readUtfInput(sourceFile.getCanonicalPath(), "ISO-8859-1").equals("árvíztűrő tükörfúrógép"), is(false));
    }
    ...
    public void testWriteDataToOutputStream() {
        ...
        //When
        new iostream.StreamReadWrite().writeDataToOutputStream(data, fileName);
        //Then
    ...
    public void testReadDataFromInputStream() {
        ...
        //When
        new iostream.StreamReadWrite().writeDataToOutputStream(data, fileName);
        List<Double> numbers = new iostream.StreamReadWrite().readDataFromInputStream(fileName);
        //Then
